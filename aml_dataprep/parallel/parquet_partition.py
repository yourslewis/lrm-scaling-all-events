#!/usr/bin/env python3
"""Convert a deterministic partition of raw shards to seqview parquet parts."""
...
def main() -> None:
    ...
    script = Path(__file__).resolve().parent / "parquet_one_shard.py"
    ...
    for _, row in selected:
        subprocess.check_call([
            ...
            args.mode,
            "--bucket_cache_size",
            str(args.bucket_cache_size),
        ])
    ready_path = write_ready_manifest(Path(args.output_dir) / "_ready", f"parquet_shard_{args.shard_index:04d}.json", {
        "stage": "parquet",
    ...
    print(f"parquet partition {args.shard_index}/{args.num_shards}: {len(selected)} rows; ready={ready_path}", flush=True)
