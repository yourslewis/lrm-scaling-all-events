...
class TrainIterableDataset(IterableDataset):
    ...
    def _process_row(self, row) -> Dict[str, torch.Tensor]:
        user_id, ad_ids, timestamps = row.user_id, list(row.ad_ids), list(row.timestamps)
        input, timestamps, label = ad_ids[:-1], timestamps[:-1], ad_ids[1:]
    ...
    def __iter__(self):
        ...
        for path in self.file_paths:
            with self.fs.open(path) as f:
                df = pd.read_parquet(f, columns=['user_id', 'ad_ids', 'timestamps'])
