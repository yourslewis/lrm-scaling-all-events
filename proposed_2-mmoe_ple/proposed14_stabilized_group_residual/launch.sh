...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} AZUREML_RUN_ID=${AZUREML_RUN_ID:-p14_stabilized_group_residual_$(date +%Y%m%d)} \
  torchrun --nproc_per_node=1 --master_port=${MASTER_PORT:-30114} main.py \
  --gin_config_file="$ROOT/proposed_2-mmoe_ple/config/proposed14_stabilized_group_residual.gin" \
  --output_path="$ROOT/results_v2/proposed14_stabilized_group_residual" \
