...
def inference_embeddings(
    ...
    if tokenizer_name != '':
        ...
        # TODO: access private attribute here, need to refactor later
        tokenizer = model._embedding_module.model.tokenizer
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, total=len(dataloader), desc="Encoding"):
            indices = batch["InferUserId"]
            lengths = batch["lengths"].to(device)  # [B]
            timestamps = batch["timestamps"].to(device)  # [B]
            inputs = batch["inputs"].to(device)  # [B, L, T]

            # Create fake past_ids here, it could be any value other than padded_token_id=0.
            # This is because positional embedding module uses padded_token_id to create mask internally
            B, L, _ = inputs.shape
            arange = torch.arange(L).unsqueeze(0).to(device)        # [1, L]
            mask = arange < lengths.unsqueeze(1)         # [B, L], bool
            fake_past_ids = mask.long()                 # Convert bool to 0/1

            past_embeddings = model._embedding_module.get_item_embeddings(inputs)   # [B, L, D]

            # print(f"inputs shape: {inputs.shape}")
            # print(f"past emb shape: {past_embeddings.shape}")
            batch_emb = model.encode(                                # [B, D]
                past_lengths=lengths,              # [B]     ``
                past_ids=fake_past_ids,                   # fake past ids, [B, L]
    ...
    out_path = f"{output_dir}/shard_{rank}_user_emb.tsv"
    final_df.to_csv(out_path, sep='\t', index=False)
    print(f"[Rank {rank}] Saved embeddings to {out_path}")


def make_user_encoder(
    ckpt_path: str = "",
...
def main(argv) -> None:  # pyre-ignore [2]
    ...
    print("load model")
    model = make_user_encoder(ckpt_path=ckpt_path, pinsage_ckpt_path=pinsage_ckpt_path)
