...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} AZUREML_RUN_ID=${AZUREML_RUN_ID:-proposed17_hierarchical_group_expert_residual_20260510} \
  torchrun --nproc_per_node=1 --master_port=${MASTER_PORT:-30250} main.py \
  --gin_config_file="$ROOT/proposed_2-mmoe_ple/config/proposed17_hierarchical_group_expert_residual.gin" \
  --output_path="$ROOT/results_v2/proposed17_hierarchical_group_expert_residual" \
