...
class LearnablePositionalEmbeddingInputFeaturesPreprocessor(
    ...
    def forward(
        ...
        user_embeddings = past_embeddings * (self._embedding_dim**0.5) + self._pos_emb(   # [B, N, D]
...
class LearnablePositionalEmbeddingRatedInputFeaturesPreprocessor(
    ...
    def __init__(
        ...
        self._fusion_proj = torch.nn.Linear(item_embedding_dim+rating_embedding_dim, item_embedding_dim)
        self.reset_state()
    ...
    def forward(
        ...
        fused_emb = self._fusion_proj(concat_emb) * (self._fusion_proj.out_features ** 0.5)
        user_embeddings = fused_emb + self._pos_emb(torch.arange(N, device=past_ids.device).unsqueeze(0).repeat(B, 1))
        user_embeddings = self._emb_dropout(user_embeddings)
...
class LearnablePositionalEmbeddingEventTypeEmbeddingInputFeaturesPreprocessor(
    ...
    def reset_state(self) -> None:
        truncated_normal(
            self._pos_emb.weight.data,
            mean=0.0,
            std=math.sqrt(1.0 / self._hidden_size),
        )
        truncated_normal(
