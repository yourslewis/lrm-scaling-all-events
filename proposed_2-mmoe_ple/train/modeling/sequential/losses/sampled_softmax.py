...
class SampledSoftmaxLoss(AutoregressiveLoss):
    ...
    def jagged_forward(  # pyre-ignore [15]
        ...
        **kwargs,
        ...
        pos_sim = positive_logits.squeeze(1) * self._softmax_temperature  # [N']
        neg_sim = sampled_negatives_logits.mean(dim=1) * self._softmax_temperature  # [N']

        metrics = {
            "positive_similarity": pos_sim.mean().detach(),
            "negative_similarity": neg_sim.mean().detach(),
            "similarity_gap": (pos_sim - neg_sim).mean().detach(),
        }

        return (
            jagged_loss * supervision_weights
        ).sum() / supervision_weights.sum(), aux_losses, metrics
    ...
    def forward(  # pyre-ignore [15]
        ...
        **kwargs,
        ...
        # Call `process_batch` once per batch to cache the valid candidate pool
        negatives_sampler.process_batch(supervision_ids, supervision_ids!=0, supervision_embeddings)
