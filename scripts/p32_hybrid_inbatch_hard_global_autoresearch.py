...
CFGDIR = ROOT / "proposed_2-mmoe_ple/config/generated_p32_hybrid_inbatch_hard_global"
OUTROOT = ROOT / "results_v2"
RUNNER_DIR = OUTROOT / "p32_hybrid_inbatch_hard_global_autoresearch"
PY = os.environ.get("LRM_PYTHON", "/home/yourslewis/miniconda3/envs/hstu/bin/python3.10")
MAX_BATCH = int(os.environ.get("P32_MAX_BATCH", "26000"))
POLL = int(os.environ.get("P32_POLL_SECONDS", "60"))
MAX_PARALLEL = int(os.environ.get("P32_MAX_PARALLEL", "2"))
RUNS = [
    ("p32a_hybrid_inbatch50_hard30_global20_n32", 0.30),
]
...
def start_run(spec: tuple[str, float], gpu: int, slot: int):
    ...
    proc = subprocess.Popen(
        ...
        cwd=TRAIN,
        env=env_for(gpu, 33300 + slot + int(hard_fraction * 100), run),
        stdout=handle,
...
def main() -> None:
    ...
    state = {
        ...
        "finished": [],
        "started_at": utc(),
        "note": "P32 hybrid in-batch/hard/global negatives; eval logic unchanged.",
    }
