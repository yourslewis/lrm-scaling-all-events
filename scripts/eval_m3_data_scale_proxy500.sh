...
SCALE_TAG=${SCALE_TAG:-25pct}
TRAIN_RUN=${TRAIN_RUN:-$ROOT/results_v2/m3_data_scale/m3_data_scale_${SCALE_TAG}}
PROXY_BASE=/home/yourslewis/lrm_benchmarkv4/processed/m3_data_scale_${SCALE_TAG}_proxy_500_seed1
...
GPU_ID=${GPU_ID:-1}
LOCK=$PROXY_BASE/m3_data_scale_${SCALE_TAG}_eval3h_500bank.lock
LOG_DIR=$PROXY_BASE/m3_data_scale_${SCALE_TAG}_eval3h_500bank_logs
mkdir -p "$LOG_DIR"
LOG=$LOG_DIR/driver_$(date -u +%Y%m%dT%H%M%SZ).log
exec >>"$LOG" 2>&1

echo "START $(date -u +%Y-%m-%dT%H:%M:%SZ) scale=$SCALE_TAG device=$DEVICE [500-BANK]"
if mkdir "$LOCK" 2>/dev/null; then
  trap 'rmdir "$LOCK" 2>/dev/null || true' EXIT
else
  echo "SKIP: another M3 ${SCALE_TAG} [500-BANK] eval is active ($LOCK)"
  exit 0
fi

if pgrep -u yourslewis -f "m3_data_scale_${SCALE_TAG}_eval3h_500bank_.*(encode_target_queries|encode_documents|score_encoded_proxy)" >/dev/null; then
  echo "SKIP: existing M3 ${SCALE_TAG} [500-BANK] eval process active"
  exit 0
fi
...
# M3 launcher writes real artifacts under $TRAIN_RUN/<AZUREML_RUN_ID>/; resolve latest child if needed.
if [ ! -f "$VAL" ]; then
  CHILD_RUN=$(find "$TRAIN_RUN" -mindepth 1 -maxdepth 1 -type d -name "m3_data_scale_${SCALE_TAG}_*" -printf "%T@ %p\n" 2>/dev/null | sort -nr | head -1 | cut -d" " -f2- || true)
...
RUN_ID=m3_data_scale_${SCALE_TAG}_eval3h_500bank_b${BATCH_PAD}
RUN_ROOT=$PROXY_BASE/${RUN_ID}_$(date -u +%Y%m%dT%H%M%SZ)
DONE_MARKER=$PROXY_BASE/m3_data_scale_${SCALE_TAG}_eval3h_500bank_done_batches.txt
...
SOURCE_ROOT=$ROOT
GIN_CONFIG=$ROOT/proposed_2-mmoe_ple/config/generated_m3_data_scale/m3_aux_light_hstu_newdata_datascale.gin
...
MODEL_SUBMISSION_ID=m3_data_scale_${SCALE_TAG}.${BATCH_PAD}.proxy500
PREDICTION_RUN_ID=$RUN_ID

{
  echo "title=M3 data-scale ${SCALE_TAG} [500-BANK] proxy eval"
...
printf '{"stage":"encode_target_queries","scale":"%s","batch":"%s","bank_count":500,"at":"%s"}\n' "$SCALE_TAG" "$BATCH_PAD" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$RUN_ROOT/progress.jsonl"
...
printf '{"stage":"encode_documents","scale":"%s","batch":"%s","bank_count":500,"at":"%s"}\n' "$SCALE_TAG" "$BATCH_PAD" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$RUN_ROOT/progress.jsonl"
...
printf '{"stage":"score_encoded_proxy","scale":"%s","batch":"%s","bank_count":500,"at":"%s"}\n' "$SCALE_TAG" "$BATCH_PAD" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$RUN_ROOT/progress.jsonl"
...
printf '{"stage":"done","scale":"%s","batch":"%s","bank_count":500,"at":"%s"}\n' "$SCALE_TAG" "$BATCH_PAD" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$RUN_ROOT/progress.jsonl"
echo "$BATCH_PAD" >> "$DONE_MARKER"
echo "$RUN_ROOT" > "$PROXY_BASE/latest_m3_data_scale_${SCALE_TAG}_eval3h_500bank_run_dir.txt"
echo "DONE [500-BANK] scale=$SCALE_TAG run_root=$RUN_ROOT"
