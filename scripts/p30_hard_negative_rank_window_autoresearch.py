...
CFGDIR = ROOT / "proposed_2-mmoe_ple/config/generated_p30_hard_negative_rank_window"
OUTROOT = ROOT / "results_v2"
RUNNER_DIR = OUTROOT / "p30_hard_negative_rank_window_autoresearch"
PY = os.environ.get("LRM_PYTHON", "/home/yourslewis/miniconda3/envs/hstu/bin/python3.10")
MAX_BATCH = int(os.environ.get("P30_MAX_BATCH", "26000"))
POLL = int(os.environ.get("P30_POLL_SECONDS", "60"))
MAX_PARALLEL = int(os.environ.get("P30_MAX_PARALLEL", "2"))
RUNS = [
    ("p30a_hardmix_safe_r128_1024_n32", 0.25),
    ("p30b_hardmix_aggr_r8_256_n32", 0.25),
]
...
def start_run(spec: tuple[str, float], gpu: int, slot: int):
    ...
    proc = subprocess.Popen(
        ...
        cwd=TRAIN,
        env=env_for(gpu, 33100 + slot + int(hard_fraction * 100), run),
        stdout=handle,
...
def main() -> None:
    ...
    state = {
        ...
        "finished": [],
        "started_at": utc(),
        "note": "P30 hard-negative rank-window sweep; eval logic unchanged.",
    }
