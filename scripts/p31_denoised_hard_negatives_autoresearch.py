...
CFGDIR = ROOT / "proposed_2-mmoe_ple/config/generated_p31_denoised_hard_negatives"
OUTROOT = ROOT / "results_v2"
RUNNER_DIR = OUTROOT / "p31_denoised_hard_negatives_autoresearch"
PY = os.environ.get("LRM_PYTHON", "/home/yourslewis/miniconda3/envs/hstu/bin/python3.10")
MAX_BATCH = int(os.environ.get("P31_MAX_BATCH", "26000"))
POLL = int(os.environ.get("P31_POLL_SECONDS", "60"))
MAX_PARALLEL = int(os.environ.get("P31_MAX_PARALLEL", "2"))
RUNS = [
    ("p31a_denoised_hardmix_f025_n32", 0.25),
    ("p31b_denoised_hardmix_f035_n32", 0.35),
]
...
def start_run(spec: tuple[str, float], gpu: int, slot: int):
    ...
    proc = subprocess.Popen(
        ...
        cwd=TRAIN,
        env=env_for(gpu, 33200 + slot + int(hard_fraction * 100), run),
        stdout=handle,
...
def main() -> None:
    ...
    state = {
        ...
        "finished": [],
        "started_at": utc(),
        "note": "P31 denoised hard negatives; eval logic unchanged.",
    }
